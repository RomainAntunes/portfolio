...
export class HomeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.projects = this.projectsService.getProjects().slice(0, 6);
  ...
  onOrientationChange(event: DeviceOrientationEvent) {
    ...
    let rotateYpx = Math.min(
      Math.max(rotateY, -this.MAX_MOVEMENT),
      this.MAX_MOVEMENT,
    ).toFixed(2);
    let rotateXpx = Math.min(
      Math.max(rotateX, -this.MAX_MOVEMENT),
      this.MAX_MOVEMENT,
    ).toFixed(2);
